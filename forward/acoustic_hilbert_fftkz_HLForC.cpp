...
int main(int argc, char *argv[])
{
	...
	float *migration_ppdu;
	float *migration_ppud;
	...
	for(i=0;i<GPU_N;i++)
	{
		plan[i].coef=(float*)malloc(sizeof(float)*nfftz);

		plan[i].seismogram_obs=(float*)malloc(sizeof(float)*itmax*rnmax);
		plan[i].seismogram_up=(float*)malloc(sizeof(float)*itmax*rnmax);
		plan[i].seismogram_down=(float*)malloc(sizeof(float)*itmax*rnmax);
		plan[i].seismogram_dir=(float*)malloc(sizeof(float)*itmax*rnmax);
		plan[i].seismogram_dirup=(float*)malloc(sizeof(float)*itmax*rnmax);
		plan[i].seismogram_dirdown=(float*)malloc(sizeof(float)*itmax*rnmax);
		...
		plan[i].seismogram_hrms=(float*)malloc(sizeof(float)*itmax*rnmax);
/*
...
*/
	...
	for(i=0;i<GPU_N;i++)
	{
		plan[i].image_ppdu=(float*)malloc(sizeof(float)*ntp);
		plan[i].image_ppud=(float*)malloc(sizeof(float)*ntp);
	...
	migration_ppdu=(float*)malloc(sizeof(float)*np);
	migration_ppud=(float*)malloc(sizeof(float)*np);
	...
	//////////////////////////////////////////////////////////////
	float dkz=1.0/(nfftz*dz);// wavenumber interval
	float kcut=1.0/15.0/dz;//cut-off wavenumber
	int layerz=kcut/dkz; // ~~ 1/10*ntz

	if(myid==0)
		printf("\nDamping wavenumber point No. = %d; NFFTz = %d\n\n",layerz,nfftz);
	if(myid==0&&layerz>nfftz/6)
	{printf("Damping wavenumber is to long !\nBreak up!\n");
		return(0);}

	int upz=layerz;
	int downz=nfftz-layerz;
	for(i=0;i<GPU_N;i++)
	{
		for(iz=0;iz<nfftz;iz++)
		{
			if(iz<=upz)
				plan[i].coef[iz]=0.5000-0.5*cos(PI*iz/layerz);
			if(iz>upz&&iz<downz)
				plan[i].coef[iz]=1.0;
			if(iz>=downz)
				plan[i].coef[iz]=0.5000+0.5*cos(PI*(iz-downz)/layerz);
		}
	}
	//////////////////////////////////////////////////////////////
	/*=========================================================
	...
	/*	if(scatter_flag)
		ricker_phase_shift(itmax,rick);

		fp=fopen("./output/rick_shift.bin","wb");    
		fwrite(&rick[0],sizeof(float),itmax,fp);
		fclose(fp);*/
	}
	...
	hilbert_transform1d(rick, hrick, itmax);

	fp=fopen("../output/rick_hilbert.bin","wb");
	fwrite(&hrick[0],sizeof(float),itmax,fp);
	fclose(fp);
	...
	if(myid==0)
	{
		...
		fclose(fp);
		fp=fopen("../output/acc_vpful.bin","wb");
		fwrite(&vp[0],sizeof(float),ntp,fp);
		fclose(fp);
	...
	if(myid==0)
	{
		printf("====================\n");
		if(scatter_flag)
			printf("    The Scatter Flag == %d--- PSF\n",scatter_flag);
		else
			printf("    The Scatter Flag == %d--- NORMAL\n",scatter_flag);
		printf("====================\n");
	}
	...
	for(ip=0;ip<np;ip++)
	{
		tmp1[ip]=0.0;
		tmp3[ip]=0.0;
		migration_ppdu[ip]=0.0;
		migration_ppud[ip]=0.0;
	}
	...
	if(scatter_flag==0)
	{
		...
		for(iss=0;iss<eachsid;iss++)
		{
			...
			for(i=0;i<GPU_N;i++)
			{
				...
				sprintf(filename,"../output/%dsource_seismogram_obs.bin",is+i+1);
				...
				fp=fopen(filename,"wb");
				fwrite(&plan[i].seismogram_up[0],sizeof(float),ss[is+i].r_n*itmax,fp);
				fclose(fp);

				sprintf(filename,"../output/%dsource_seismogram_obsdown.bin",is+i+1);
				fp=fopen(filename,"wb");
				fwrite(&plan[i].seismogram_down[0],sizeof(float),ss[is+i].r_n*itmax,fp);
				fclose(fp);
			}//end GPU
		}//end is (shotnumbers)
	}//end scatter_flag
	else  // scatter_flag==1
	{
		...
		///////////////////////////////////////////////
		if(myid==0)
			printf("Obtain the background scatter seismogram !\n");
		...
		for(iss=0;iss<eachsid;iss++)
		{
			...
			{
				//// output the seismogram ////
				sprintf(filename,"../output/%dsource_seismogram_sobs.bin",is+i+1);
				fp=fopen(filename,"wb");
				fwrite(&plan[i].seismogram_obs[0],sizeof(float),ss[is+i].r_n*itmax,fp);
				fclose(fp);

				sprintf(filename,"../output/%dsource_seismogram_sobsup.bin",is+i+1);
				fp=fopen(filename,"wb");
				fwrite(&plan[i].seismogram_up[0],sizeof(float),ss[is+i].r_n*itmax,fp);
				fclose(fp);

				sprintf(filename,"../output/%dsource_seismogram_sobsdown.bin",is+i+1);
				fp=fopen(filename,"wb");
				fwrite(&plan[i].seismogram_down[0],sizeof(float),ss[is+i].r_n*itmax,fp);
				fclose(fp);
			}//end GPU
		...
		if(myid==0)
			printf("The scatter model is done\n"); 

		for(ix=pmlc+swin/2;ix<ntx-pmlc-swin/2;ix++)
		{
			for(iz=pmlc+swin/2;iz<ntz-pmlc-5;iz++)
			{
				if((ix-(pmlc+swin/2))%swin==0&&(iz-(pmlc+swin/2))%swin==0)
					rho[ix*ntz+iz]+=vp[ix*ntz+iz]*1.0e-3; // Scatters
					//vp[ix*ntz+iz]*=1.0001; // Scatters
			}
		}

		fp=fopen("../output/ini_scatter_vp.bin","wb");
		for(ix=pmlc;ix<=ntx-pmlc-1;ix++)
		{
			for(iz=pmlc;iz<=ntz-pmlc-1;iz++)
			{
		...
		fp=fopen("../output/rho_scatter.bin","wb");
		for(ix=pmlc;ix<=ntx-pmlc-1;ix++)
		{
			for(iz=pmlc;iz<=ntz-pmlc-1;iz++)
			{
		...
		///////////////////////////////////////////////
		if(myid==0)
			printf("Obtain the observed scatter seismogram !\n");
		...
		for(iss=0;iss<eachsid;iss++)
		{
			...
			for(i=0;i<GPU_N;i++)
			{
				...
				fread(&plan[i].seismogram_dir[0],sizeof(float),ss[is+i].r_n*itmax,fp);
				fclose(fp);

				sprintf(filename,"../output/%dsource_seismogram_sobsup.bin",is+i+1);
				fp=fopen(filename,"rb");
				fread(&plan[i].seismogram_dirup[0],sizeof(float),ss[is+i].r_n*itmax,fp);
				fclose(fp);

				sprintf(filename,"../output/%dsource_seismogram_sobsdown.bin",is+i+1);
				fp=fopen(filename,"rb");
				fread(&plan[i].seismogram_dirdown[0],sizeof(float),ss[is+i].r_n*itmax,fp);
				fclose(fp);
				//////////////////////////////////////////////////////
				for(ip=0;ip<ss[is+i].r_n*itmax;ip++)
				{
					plan[i].seismogram_rms[ip]=plan[i].seismogram_obs[ip]-plan[i].seismogram_dir[ip];
					plan[i].seismogram_rmsup[ip]=plan[i].seismogram_up[ip]-plan[i].seismogram_dirup[ip];
					plan[i].seismogram_rmsdown[ip]=plan[i].seismogram_down[ip]-plan[i].seismogram_dirdown[ip];
				}
				//// output the seismogram ////
				sprintf(filename,"../output/%dsource_seismogram_scatter.bin",is+i+1);
				fp=fopen(filename,"wb");
				...
				fp=fopen(filename,"wb");
				fwrite(&plan[i].seismogram_rmsup[0],sizeof(float),ss[is+i].r_n*itmax,fp);
				fclose(fp);

				sprintf(filename,"../output/%dsource_seismogram_scatterdown.bin",is+i+1);
				fp=fopen(filename,"wb");
				fwrite(&plan[i].seismogram_rmsdown[0],sizeof(float),ss[is+i].r_n*itmax,fp);
				fclose(fp);
			}//end GPU
		}//end is (shotnumbers)
	}//end scatter_flag
	...
	for(i=0;i<GPU_N;i++)
	{
		...
		free(plan[i].seismogram_up);
		free(plan[i].seismogram_down);
		free(plan[i].seismogram_dir);
		free(plan[i].seismogram_dirup);
		free(plan[i].seismogram_dirdown);
		free(plan[i].seismogram_syn); 
		free(plan[i].seismogram_rms);
		free(plan[i].seismogram_rmsup);
		free(plan[i].seismogram_rmsdown);
		free(plan[i].seismogram_hrms);
		/*
		 ...
		 */
		free(plan[i].image_ppdu);
		free(plan[i].image_ppud);
...
void get_acc_model(float *vp, float *rho, int ntp, int ntx, int ntz, int pml, char *v_file)
{
	...
	//sprintf(filename,"./input/%s",v_file);
...
void ini_model_minea(float *vp, float *vp_n, int ntp, int ntz, int ntx, int pml, int window)
{
	...
	FILE *fp;
	printf("The summation of Coef is : %f\n",csum);
	fp=fopen("../output/coef.bin","wb");
	fwrite(&coef[0],sizeof(double),windxz*windxz,fp);
	fclose(fp);
	/////////////////////////////////////////
